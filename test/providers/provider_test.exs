defmodule GenAI.Providers.ProviderTest do
  ...
  describe "Validate TestProvider" do
    ...
    test "Acceptance Test - VNext - use provider" do
      ...
      {:ok, {messages, thread}} = GenAI.ThreadProtocol.effective_messages(thread, model, context, options)
      
      [_,user_message|_] = messages
      assert user_message.content == "Hello"
      assert user_message.role == :user
      
      {:ok, {model_settings, thread}} = GenAI.ThreadProtocol.effective_model_settings(thread, model, context, options)
      assert model_settings[:apple] == :go
      
      {:ok, {safety_settings, thread}} = GenAI.ThreadProtocol.effective_safety_settings(thread, context, options)
