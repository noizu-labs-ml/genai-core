defimpl GenAI.Thread.LegacyStateProtocol, for: GenAI.Thread.Standard do
  ...
  defp effective_value_fetch_success(response, thread_context), do: {:ok, {response, thread_context}}
  ...
  def apply_setting(thread_context, node) do
    thread_context
    |> update_in([Access.key(:state), Access.key(:settings), node.setting], & [node.value | (&1 || []) ])
    |> ok()
  end
  ...
  def apply_tools(thread_context, tools) when is_list(tools) do
    tools
    |> Enum.reduce_while({:ok, thread_context},
         fn
           tool, {:ok, thread_context} -> {:cont, GenAI.Thread.LegacyStateProtocol.apply_tool(thread_context, tool)}
           _, error -> {:halt, error}
         end
       )
  end
  ...
  def apply_message(thread_context, message) do
    thread_context
    |> update_in(
         [Access.key(:state), Access.key(:messages)], & [message | (&1 || []) ]
       )
    |> ok()
  end
  ...
  def apply_messages(thread_context, messages) when is_list(messages) do
    thread_context
    |> update_in(
         [Access.key(:state), Access.key(:messages)], & Enum.reverse(messages) ++ (&1 || [])
       )
    |> ok()
  end
