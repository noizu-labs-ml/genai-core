defmodule GenAI.Graph.Root do
  ...
  def nearest_handle_entry(%__MODULE__{} = this, handle, base = element_lookup(path: under_path)) do
    ...
    local_entries = this.lookup_table.by_handle[graph_handle(scope: :local, name: handle)] || []
    cond do
      x = if_element_under_path(this, global_entry, base) -> x
      x = if_element_under_path(this, standard_entry, base) -> x
      :else ->
        {_, entry}  = local_entries
                      |> Enum.reduce(
                           {@max_depth, nil},
                           fn
                             {entry_path, entry_element}, b = {b_depth, _} ->
                               e_depth = length(entry_path)
                               cond do
                                 e_depth < b_depth && List.starts_with?(entry_path, under_path) -> {e_depth, entry_element}
                                 :else -> b
                               end
                           end
                         )
  ...
  def graph_context_by_link(this, %GenAI.Graph.Link{target: connector(node: target)} = link, from_element) do
